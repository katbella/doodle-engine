...
import type { AssetManifest, AssetLoadingState, AssetLoader } from '@doodle-engine/core'
...
export function AssetProvider({
  ...
  const loaderRef = useRef<AssetLoader>(loaderProp ?? createAssetLoader(manifest.version))
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const gameTotal = manifest.totalSize - manifest.shellSize || gamePaths.length
  ...
  const getAssetUrl = useCallback((path: string) => loader.getUrl(path), [loader])

  const isReady = useCallback((path: string) => readyPaths.has(path), [readyPaths])

  const prefetch = useCallback((paths: string[]) => loader.prefetch(paths), [loader])
