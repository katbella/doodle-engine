...
describe('Condition Evaluators', () => {
  describe('hasFlag', () => {
    ...
    it('should return false when flag is not set', () => {
      const condition: Condition = { type: 'hasFlag', flag: 'doorLocked' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when flag does not exist', () => {
      const condition: Condition = { type: 'hasFlag', flag: 'nonexistent' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('notFlag', () => {
    it('should return true when flag is not set', () => {
      const condition: Condition = { type: 'notFlag', flag: 'doorLocked' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when flag is set', () => {
      const condition: Condition = { type: 'notFlag', flag: 'metBartender' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return true when flag does not exist', () => {
      const condition: Condition = { type: 'notFlag', flag: 'nonexistent' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })
  })
  ...
  describe('hasItem', () => {
    it('should return true when item is in inventory', () => {
      const condition: Condition = { type: 'hasItem', itemId: 'rusty_key' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })
  ...
  describe('variableEquals', () => {
    it('should return true when numeric variable equals value', () => {
      const condition: Condition = { type: 'variableEquals', variable: 'gold', value: 100 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return true when string variable equals value', () => {
      const condition: Condition = { type: 'variableEquals', variable: 'playerName', value: 'Hero' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when variable does not equal value', () => {
      const condition: Condition = { type: 'variableEquals', variable: 'gold', value: 50 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when variable does not exist', () => {
      const condition: Condition = { type: 'variableEquals', variable: 'nonexistent', value: 100 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('variableGreaterThan', () => {
    it('should return true when variable is greater', () => {
      const condition: Condition = { type: 'variableGreaterThan', variable: 'gold', value: 50 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when variable is equal', () => {
      const condition: Condition = { type: 'variableGreaterThan', variable: 'gold', value: 100 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when variable is less', () => {
      const condition: Condition = { type: 'variableGreaterThan', variable: 'gold', value: 200 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when variable is a string', () => {
      const condition: Condition = { type: 'variableGreaterThan', variable: 'playerName', value: 10 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('variableLessThan', () => {
    it('should return true when variable is less', () => {
      const condition: Condition = { type: 'variableLessThan', variable: 'gold', value: 200 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when variable is equal', () => {
      const condition: Condition = { type: 'variableLessThan', variable: 'gold', value: 100 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when variable is greater', () => {
      const condition: Condition = { type: 'variableLessThan', variable: 'gold', value: 50 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('atLocation', () => {
    it('should return true when at the specified location', () => {
      const condition: Condition = { type: 'atLocation', locationId: 'tavern' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when at a different location', () => {
      const condition: Condition = { type: 'atLocation', locationId: 'market' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('questAtStage', () => {
    it('should return true when quest is at the specified stage', () => {
      const condition: Condition = { type: 'questAtStage', questId: 'odd_jobs', stageId: 'started' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when quest is at a different stage', () => {
      const condition: Condition = { type: 'questAtStage', questId: 'odd_jobs', stageId: 'complete' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when quest has not been started', () => {
      const condition: Condition = { type: 'questAtStage', questId: 'unknown_quest', stageId: 'started' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('characterAt', () => {
    it('should return true when character is at the specified location', () => {
      const condition: Condition = { type: 'characterAt', characterId: 'bartender', locationId: 'tavern' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when character is at a different location', () => {
      const condition: Condition = { type: 'characterAt', characterId: 'bartender', locationId: 'market' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when character does not exist', () => {
      const condition: Condition = { type: 'characterAt', characterId: 'unknown', locationId: 'tavern' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('characterInParty', () => {
    it('should return true when character is in party', () => {
      const condition: Condition = { type: 'characterInParty', characterId: 'pixel_the_dog' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when character is not in party', () => {
      const condition: Condition = { type: 'characterInParty', characterId: 'bartender' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('relationshipAbove', () => {
    it('should return true when relationship is above value', () => {
      const condition: Condition = { type: 'relationshipAbove', characterId: 'pixel_the_dog', value: 5 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when relationship equals value', () => {
      const condition: Condition = { type: 'relationshipAbove', characterId: 'bartender', value: 5 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when relationship is below value', () => {
      const condition: Condition = { type: 'relationshipAbove', characterId: 'merchant', value: 0 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('relationshipBelow', () => {
    it('should return true when relationship is below value', () => {
      const condition: Condition = { type: 'relationshipBelow', characterId: 'merchant', value: 0 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when relationship equals value', () => {
      const condition: Condition = { type: 'relationshipBelow', characterId: 'bartender', value: 5 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when relationship is above value', () => {
      const condition: Condition = { type: 'relationshipBelow', characterId: 'pixel_the_dog', value: 5 }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
  ...
  describe('timeIs', () => {
    it('should return true when time is within normal range', () => {
      const condition: Condition = { type: 'timeIs', startHour: 9, endHour: 17 }
      const state = createTestState() // hour is 14
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when time is outside normal range', () => {
      const condition: Condition = { type: 'timeIs', startHour: 18, endHour: 22 }
      const state = createTestState() // hour is 14
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should handle wrap-around range (night time)', () => {
      const condition: Condition = { type: 'timeIs', startHour: 20, endHour: 6 }
      const state = { ...createTestState(), currentTime: { day: 1, hour: 2 } }
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should handle wrap-around range boundary', () => {
      const condition: Condition = { type: 'timeIs', startHour: 20, endHour: 6 }
      const state = { ...createTestState(), currentTime: { day: 1, hour: 20 } }
      expect(evaluateCondition(condition, state)).toBe(true)
    })
  })
  ...
  describe('itemAt', () => {
    it('should return true when item is at the specified location', () => {
      const condition: Condition = { type: 'itemAt', itemId: 'sword', locationId: 'armory' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(true)
    })

    it('should return false when item is at a different location', () => {
      const condition: Condition = { type: 'itemAt', itemId: 'sword', locationId: 'tavern' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })

    it('should return false when item does not exist', () => {
      const condition: Condition = { type: 'itemAt', itemId: 'unknown', locationId: 'tavern' }
      const state = createTestState()
      expect(evaluateCondition(condition, state)).toBe(false)
    })
  })
