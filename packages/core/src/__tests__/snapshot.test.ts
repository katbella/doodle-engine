...
describe('Snapshot Builder', () => {
    describe('buildSnapshot', () => {
        ...
        it('should not include items in inventory as itemsHere', () => {
            ...
            expect(
                snapshot.itemsHere.find((i) => i.id === 'letter')
            ).toBeUndefined();
        });
        ...
        it('should include active quests with resolved text', () => {
            ...
            expect(snapshot.quests[0].currentStageDescription).toBe(
        ...
        it('should include map when enabled', () => {
            ...
            expect(snapshot.map?.locations.find((l) => l.isCurrent)?.id).toBe(
                'tavern'
            );
        });
