...
function makeManifest(overrides?: Partial<AssetManifest>): AssetManifest {
  return {
    version: 'test-1',
    shell: [
      { path: '/assets/images/logo.png', type: 'image', size: 1024, tier: 1 },
      { path: '/assets/audio/splash.ogg', type: 'audio', size: 2048, tier: 1 },
    ],
    game: [
      { path: '/assets/images/tavern.jpg', type: 'image', size: 40000, tier: 2 },
      { path: '/assets/audio/music.ogg', type: 'audio', size: 300000, tier: 2 },
    ],
    shellSize: 3072,
    totalSize: 343072,
    ...overrides,
...
describe('generateServiceWorker', () => {
  ...
  it('works with empty manifests', () => {
    const manifest = makeManifest({ shell: [], game: [], shellSize: 0, totalSize: 0 })
    const source = generateServiceWorker(manifest)
