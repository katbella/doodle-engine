---
title: Doodle Studio
description: Build, inspect, and playtest Doodle Engine projects in the desktop editor.
---

import ThemeScreenshot from '../../../components/ThemeScreenshot.astro';
import { LinkButton } from '@astrojs/starlight/components';

Doodle Studio is the desktop app for creating a Doodle Engine game. Use it to create a project, write and organize game content, import assets, test story paths, preview the game as players will see it, and build the files you will publish.

<LinkButton href="https://github.com/katbella/doodle-engine/releases/latest" variant="primary" icon="download">Download Doodle Studio</LinkButton>

<ThemeScreenshot
    src="/images/studio/welcome.webp"
    alt="Doodle Studio welcome screen"
/>

## Start Studio

[Install Doodle Studio](/getting-started/installation/#doodle-studio-setup) if you have not set it up yet. Then launch it from your applications or Start menu.

The Welcome screen provides three starting points:

- **New project…** creates a new playable game project.
- **Open project…** opens an existing Doodle Engine project folder.
- **Recent Projects** lists the projects you have used before.

## Make your first game

For your first game, write the English text directly in Studio. You will see the exact words that appear in the game, and you can introduce translation keys later when you need them.

1. Select **New project…** and enter a project name and game title.
2. Keep **Playable example story** selected and choose **English text with a locale starter file**.
3. Keep the default React renderer and starter styles selected, choose where to save the project, and select **Create**.
4. Under **Dialogues** in the project rail, open `bartender_greeting`.
5. Select the `start` node and change the bartender’s line. Studio saves the edit shortly after you stop typing.
6. Select **Playtest**, choose **Start at node…**, and select the `start` node in `bartender_greeting`. Your new line appears in the playback column, followed by the bartender's choices.
7. Select **Preview** to play the game in its renderer. The game opens in your browser with your edit in place.
8. Select **Build** when you are ready to create the files you will publish.

<ThemeScreenshot
    src="/images/studio/workspace.webp"
    alt="A project open in the Doodle Studio workspace"
/>

## Write in Visual mode or Source mode

Studio gives Visual mode and Source mode equal support. Visual mode provides purpose-built controls for Doodle content, while Source mode includes syntax highlighting and project-aware IntelliSense. Both edit the same project files, so you can choose one or move between them.

<ThemeScreenshot
    src="/images/studio/dialogue-intellisense.webp"
    alt="Doodle Studio suggesting project flag names in the dialogue Source editor"
/>

Prefer your own editor? Doodle projects use readable `.dlg`, YAML, TypeScript, and CSS files that you can open in VS Code or any other editor. Studio notices external file changes, while the CLI provides development, validation, and build commands for terminal-based workflows.

[Learn about Visual mode and Source mode](/studio/workspace/)

## What each part of Studio does

| Area             | Position | Purpose                                                                                        |
| ---------------- | -------- | ---------------------------------------------------------------------------------------------- |
| Top bar          | Top      | Shows project status and provides the command palette, Validate, Build, Preview, and Playtest. |
| Project rail     | Left     | Opens and organizes game content.                                                              |
| Editor           | Center   | Provides visual controls and direct file editing, plus a read-only map of dialogue paths.      |
| References panel | Right    | Shows where the selected item is used.                                                         |
| Bottom dock      | Bottom   | Holds project checks, output, state tools, and the playtester.                                 |

## Continue learning

- [Create and open projects](/studio/projects/)
- [Learn the workspace](/studio/workspace/)
- [Choose a Studio theme](/studio/themes/)
- [Write dialogue visually](/studio/dialogues/)
- [Review flags and variables](/studio/flags-and-variables/)
- [Test story paths and state](/studio/playtesting/)
- [Import images, audio, and video](/studio/assets/)
- [Add localization when your game is ready for another language](/studio/localization/)
- [Validate, preview, and build](/studio/validation-builds/)

Use [Writing Dialogues](/guides/writing-dialogues/) to learn the dialogue language. The Studio dialogue guide explains the visual controls for writing the same content.
