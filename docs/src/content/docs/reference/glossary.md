...
The settings in `content/game.yaml`: the game title and subtitle, starting location, time, flags, variables, and inventory, whether the player enters their own profile (`playerCreatesProfile`), and the optional `shell:` section for splash, loading, and title screen media. See [YAML Schemas](/reference/yaml-schemas/#gameconfig).
