...
The **asset manifest** is a JSON file generated by `doodle build` (and served dynamically by `doodle dev`) that describes every asset the game needs, organized by tier.
...
In development (`doodle dev`), the manifest is generated on-the-fly by `GET /api/manifest`. The version is always `"dev"`, so no caching occurs.

In production (`doodle build`), the manifest is written to:
